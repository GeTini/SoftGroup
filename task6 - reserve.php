        ...
        <tr>
			...
			<fieldset>
				...
				<form role="form">
					...
					<div class="form-group">
						<input type="button" id="in" value="Внести">
						<input type="button" id="out" value="Результат">
					</div>
				
				<?php
					echo '<pre>';
					
					$initialInput = '9 69485 Jack 95715 qwerty 95715 Alex 83647 M 197128 qwerty 95715 Jack 93289 Alex 95715 Alex 95715 M';
					$initialArray = explode ( ' ' , $initialInput );
					$transformedDataArray = array (  );

					// Фільтрування даних
					foreach ( $initialArray as $key => $element ) {
						$isElementNumeric = is_numeric ( $element );
						$isElementNonNumeric = !is_numeric ( $initialArray [ $key + 1 ] );
						$isValidName = strlen ( $initialArray [ $key + 1 ] ) > 1;
						if ( $isElementNumeric && $isElementNonNumeric && $isValidName ) {
							$transformedDataArray [  ] = (
								array ( $element, $initialArray [ $key + 1 ] )
							);
						}
					}

					// Пошук найбільших елементів
					$recordsArray = array (  );
					foreach ( $transformedDataArray as $row ) {
						$greatestRecord = $row;
						foreach ( $transformedDataArray as $element ) {
							if ( $greatestRecord [ 1 ] == $element [ 1 ] ) {
								if ( $greatestRecord [ 0 ] < $element [ 0 ] )
									$greatestRecord = $element;
							}
						}
						$recordsArray [  ] = $greatestRecord [ 0 ] . ' ' . ucfirst( $greatestRecord [ 1 ] );
					}

					// Пошук однакових елементів і сортування
					$recordsArray = array_unique ( $recordsArray );
					rsort ( $recordsArray, 1 );

					// Форматування і вивід даних
					foreach ( $recordsArray as $key => $record ) {
						$recordsArray [ $key ] = explode ( ' ' , $record );
						echo ( $key + 1 ) . ' місце.' . $recordsArray [ $key ][ 1 ] . ' (' . $recordsArray [ $key ][ 0 ] . ')<br>';
					}
					echo '</pre>';
				?>
